...
%global build_all		%{?build_all}%{!?build_all:0}
%global build_aarch64		%{?build_aarch64}%{!?build_aarch64:%{build_all}}
%global build_alpha		%{?build_alpha}%{!?build_alpha:%{build_all}}
%global build_arm		%{?build_arm}%{!?build_arm:%{build_all}}
%global build_arc		%{?build_arc}%{!?build_arc:%{build_all}}
%global build_avr32		%{?build_avr32}%{!?build_avr32:%{build_all}}
%global build_blackfin		%{?build_blackfin}%{!?build_blackfin:%{build_all}}
%global build_c6x		%{?build_c6x}%{!?build_c6x:%{build_all}}
%global build_cris		%{?build_cris}%{!?build_cris:%{build_all}}
%global build_frv		%{?build_frv}%{!?build_frv:%{build_all}}
%global build_h8300		%{?build_h8300}%{!?build_h8300:%{build_all}}
%global build_hppa		%{?build_hppa}%{!?build_hppa:%{build_all}}
%global build_hppa64		%{?build_hppa64}%{!?build_hppa64:%{build_all}}
%global build_ia64		%{?build_ia64}%{!?build_ia64:%{build_all}}
%global build_m68k		%{?build_m68k}%{!?build_m68k:%{build_all}}
%global build_microblaze	%{?build_microblaze}%{!?build_microblaze:%{build_all}}
%global build_mips64		%{?build_mips64}%{!?build_mips64:%{build_all}}
%global build_mn10300		%{?build_mn10300}%{!?build_mn10300:%{build_all}}
%global build_nios2		%{?build_nios2}%{!?build_nios2:%{build_all}}
%global build_openrisc		%{?build_openrisc}%{!?build_openrisc:%{build_all}}
%global build_powerpc64		%{?build_powerpc64}%{!?build_powerpc64:%{build_all}}
%global build_powerpc64le	%{?build_powerpc64le}%{!?build_powerpc64le:%{build_all}}
%global build_riscv64		%{?build_riscv64}%{!?build_riscv64:%{build_all}}
...
# number.
%global DATE 20251030
%global gitrev 04696df09633baf97cdbbdd6e9929b9d472161d3
...
# git --git-dir=gcc-dir.tmp/.git archive --prefix=%%{name}-%%{version}-%%{DATE}.tar.xz/ %%{gitrev} | xz -9e > %%{name}-%%{version}-%%{DATE}.tar.xz
# rm -rf gcc-dir.tmp
...
Patch901: cross-gcc13-format-config.patch
...
echo 'Fake Red Hat Cross %{version}-%{cross_gcc_release}' > gcc/DEV-PHASE
...
%changelog
* Fri Oct 31 2025 Andrew Tapia <andrew.tapia@uky.edu> - 14.2.0-1
- Update for GCC 14.

* Tue Jul 19 2022 Davide Cavalca <dcavalca@fedoraproject.org> - 12.1.1-2
- Relax binutils requirement for EPEL 8
